...
// RunRow, IsBotId/BotIdFor, RUN_BOT_ID, and the arcade rules (TrimRuns/TopRuns/
// BestRunFor). Shared with the CLIENT's local board so the two tables cannot
// drift apart - see the header for why that matters. Everything below is what
// only the SERVER needs: the career table, and the file on the box.
#include "runboard.h"
...
public:
    ...
    // The arcade rules themselves live in runboard.h, because the client's local
    // board has to apply exactly the same ones. These three are thin so that the
    // call sites (and the tests) keep reading the way they always did.
    void trimRuns() { TrimRuns(runs); }

    std::vector<RunRow> topRuns(bool officialOnly, size_t count) const {
        return TopRuns(runs, officialOnly, count);
    }

    bool bestRunFor(const std::string& id, bool officialOnly, RunRow& out) const {
        return BestRunFor(runs, id, officialOnly, out);
    }
